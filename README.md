...
一个纯前端的常用小工具集合，无需服务器支持，可直接在浏览器中运行。
...
- ✅ **UID提取** - 从文本或文件中提取唯一标识符
- ✅ **行号显示** - 实时显示文本行号，支持滚动同步
- ✅ **响应式设计** - 适配桌面和移动设备
- ✅ **中文本地化** - 完整的中文界面和提示
- ✅ **纯前端实现** - 无需后端服务器，数据不离开浏览器
- ✅ **多文件支持** - 支持.txt、.json、.log、.csv、.groovy、.sql等多种文件格式
...
4. 点击"提取UID"按钮，系统会自动识别并提取唯一标识符
5. 结果区域显示提取的UID列表和统计信息

### 常用操作
- **复制结果**：格式化后可一键复制结果
- **下载JSON**：将格式化结果下载为文件
- **下载TXT**：UID提取结果可下载为文本文件
- **清空输入**：快速清空所有输入内容
- **加载示例**：对比功能提供示例数据
